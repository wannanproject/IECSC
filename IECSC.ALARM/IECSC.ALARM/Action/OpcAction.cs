...
namespace IECSC.ALARM
{
    public class OpcAction
    {
        ...
        public bool AddOpcItem(ref string errMsg)
        {
            try
            {
                var items = BizHandle.Instance.alarmItems.Keys.ToArray();
        ...
        private void OpcClient_DataChanged(MSTL.OpcClient.Model.DataChangedEventArgs e)
        {
            try
            {
                foreach (var item in e.Data)
                {
                    ...
                    if(!BizHandle.Instance.alarmItems.ContainsKey(item.TagLongName))
                    {
                        continue;
                    }
                    BizHandle.Instance.alarmItems[item.TagLongName].TagValue = Convert.ToInt32(item.TagValue ?? 0);
                }
            }
            catch (Exception ex)
            {
