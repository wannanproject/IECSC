...
namespace WCS.ExcuteJob
{
    class JobBiz
    {
        ...
        public JobBiz()
        {
            ...
            //读取数据库连接字符串
            orcleConnectionString = ConfigurationManager.ConnectionStrings["Oracle"].ConnectionString;
            //初始化数据库
            this.Db = DbHelper.GetDb(orcleConnectionString, DbHelper.DataBaseType.Oracle, ref errMsg);
        }
        ...
        public IEnumerable<PsbJobStatus> GetAllJob()
        {
            try
            {
                var jobs = Db.Connection.Query<PsbJobStatus>("select t.* from psb_job_status t");
                return jobs;
            }
            catch (Exception ex)
            {
