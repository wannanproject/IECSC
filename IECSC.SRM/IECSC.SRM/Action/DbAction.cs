...
namespace IECSC.SRM
{
    public partial class DbAction
    {
        ...
        private DataTable GetSrmCmd()
        {
            ...
            {
                log.Error($"[异常]执行GetSrmCmd()获取未下发堆垛机指令:{ex.ToString()}");
                return null;
            }
        }
        /// <summary>
        ...
        public bool UpdateWarnLog(int objid)
        {
            try
            {
                ...
                sb.Append(" ERR_SECONDS = (SYSDATE - ERR_BEGIN_TIME) * 24 * 60 * 60");
                sb.Append(" WHERE OBJID = :OBJID");
                var param = new DynamicParameters();
                param.Add("OBJID", objid);
                Db.Connection.Execute(sb.ToString(), param);
        ...
        public int GetFipFlag()
        {
            ...
            {
                var sb = new StringBuilder();
                sb.Append(" SELECT NVL(MIN(T.FIP_FLAG),0) FIP_FLAG FROM PEM_CRN_FORK_STATUS T");
