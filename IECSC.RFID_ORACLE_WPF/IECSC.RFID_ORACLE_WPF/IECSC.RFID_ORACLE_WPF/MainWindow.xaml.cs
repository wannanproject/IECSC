using IECSC.RFID_ORACLE_WPF.CustomControl;
using System;
...
namespace IECSC.RFID_ORACLE_WPF
{
    ...
    public partial class MainWindow : Window
    {
        ...
        private bool InitDb(ref string errMsg)
        {
            try
            {
                ShowFormData.Instance.ShowFormInfo(new ShowInfoData("初始化数据库连接...", string.Empty));

                if (DbAction.Instance.GetDbTime())
                {
                    ShowFormData.Instance.ShowFormInfo(new ShowInfoData("已成功连接数据库！", string.Empty));
                    ShowDbConnStatus("Y");
                }
                else
                {
                    ShowFormData.Instance.ShowFormInfo(new ShowInfoData("连接数据库失败！", string.Empty));
                    ShowDbConnStatus("N");
                    return false;
                }
                return true;
        ...
        /// InitSocket
        /// </summary>
        private bool InitSocket(ref string errMsg)
        {
            try
            {
                BizHandle.Instance.mRfIdReadList = DbAction.Instance.GetIPAndPortNoFromDB();
                if (BizHandle.Instance.mRfIdReadList != null)
                {
                    return true;
                }
                else
            ...
            {
                ShowExecLog($"[异常]InitSocket失败,[原因]{ex.Message}", string.Empty);
                return false;
            }
        }
        ...
        {
            foreach (var loc in BizHandle.Instance.mRfIdReadList.OrderBy(p => p.LocNo))
            {
                var locControl = new LocControl();
                locControl.LocNo = loc.LocNo;
                locControl.LocPlcNo = loc.LocNo;
                locControl.IP = loc.IPAddress;
        ...
        /// 站台状态刷新
        /// </summary>
        private void ShowLocStatus(string msg, string locNo)
        {
            //var loc = BizHandle.Instance.mRfIdReadList.SingleOrDefault(p=>p.LocNo == locNo);
            
            if (!msg.Equals(locControlDic[locNo].LastReadInfo))
            {
                locControlDic[locNo].LastReadInfo = locControlDic[locNo].ReadInfo;
            }
            locControlDic[locNo].ReadInfo = msg;
            locControlDic[locNo].ReadTime = DateTime.Now.ToString();
