    ...
    public partial class MainWindow : Window
    {
        ...
        private void btnFinish_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                var selectItem = (TaskCmd)dgv.SelectedItem;
                var loc = new Loc();
                loc.LocNo = selectItem.ElocNo;
                loc.TaskNo = selectItem.TaskNo;

                if (loc.TaskNo == 0)
                {
                    MessageBox.Show($"未找到{loc.TaskNo}任务信息", "提示", MessageBoxButton.YesNo, MessageBoxImage.Warning);
                    return;
                }
                if (MessageBox.Show($"确定要完成任务[{loc.TaskNo}]吗？", "强制完成", MessageBoxButton.YesNo, MessageBoxImage.Information) == MessageBoxResult.Yes)
                {
                    loc.RequestFinishObjid = DbAction.Instance.GetObjidForCmdFinish();
                    loc = DbAction.Instance.RequestFinishTaskCmd(loc, 2);
                    if (loc.FaultNo == 0)
                    {
                        BizHandle.Instance.locDic[loc.LocNo].RequestFinishObjid = 0;
                        BizHandle.Instance.locDic[loc.LocNo].bizStatus = BizStatus.None;
                        ShowTaskCmd();
                        MessageBox.Show($"已强制完成任务[{loc.TaskNo}]", "提示", MessageBoxButton.YesNo, MessageBoxImage.Information);
                    }
                    else
                    {
                        MessageBox.Show($"强制完成任务[{loc.TaskNo}]失败：{loc.FaultDesc}", "异常", MessageBoxButton.YesNo, MessageBoxImage.Error);
                    }
                }
            }
            catch (Exception ex)
