...
namespace IECSC.TRANS
{
    public class OpcAction
    {
        ...
        /// 写入故障编码
        /// </summary>
        public bool WriteFaultNo(Loc loc, ref string errMsg)
        {
            try
            {
                foreach (var item in BizHandle.Instance.writeItems)
                {
                    ...
                    if (item.Value.BusIdentity.Equals("Write.FaultNo"))
                    {
                        var kValue = new KeyValuePair<string, object>(item.Key, loc.FaultNo);
        ...
        /// 写入“请求上位机下发任务”已处理信号
        /// </summary>
        public bool WriteRequestDeal(Loc loc, ref string errMsg)
        {
            try
            {
                foreach (var item in BizHandle.Instance.writeItems)
                {
                    ...
                    if(item.Value.BusIdentity.Equals("Write.DealRequest"))
                    {
                ...
                errMsg = "未找到OPC“请求上位机下发任务处理信号”写入项";
                return false;
            }
            catch (Exception ex)
            {
        ...
        /// 写入“站点有货需取货”已处理信号
        /// </summary>
        public bool WriteToLoadDeal(Loc loc, ref string errMsg)
        {
            try
            {
                foreach (var item in BizHandle.Instance.writeItems)
                {
                    ...
                    if (item.Value.BusIdentity.Equals("Write.DealToLoad"))
                    {
                        var kValue = new KeyValuePair<string, object>(item.Key, 1);
                        if (opcClient.WriteValue(McConfig.Instance.OpcGroupName, kValue, ref errMsg))
                        {
                ...
                foreach (var item in BizHandle.Instance.writeItems)
                {
                    ...
                    switch (item.Value.BusIdentity)
                    {
                        case "Write.TaskNo":
                            ...
                            break;
                        case "Write.PalletNo":
                            keyValues.Add(new KeyValuePair<string, object>(item.Key, loc.taskCmd.PalletNo.PadRight(20, ' ')));
                            break;
                        case "Write.SlocArea":
        ...
        /// <summary>
        /// 写入下位机验证信息：即只写入任务流水号、工装号
        /// </summary>
        public bool WriteCheckInfo(Loc loc, ref string errMsg)
        {
            try
            {
                if (loc.taskCmd.TaskNo == 0)
